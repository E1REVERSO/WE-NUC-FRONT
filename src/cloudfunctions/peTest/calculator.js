/*!
 * physical-test-calculator v1.1.3 
 * (c) 2017 fjc0k
...
var compare = function (score, rule, isSmallBetter) {
  return isSmallBetter ? score > rule : score < rule;
};

var calculate = function (score, standard, isSmallBetter) {
  if ( isSmallBetter === void 0 ) isSmallBetter = false;


  score = Number(score);

  var values = standard[0], rules = standard[1];

  var len = rules.length;

  for (var i = 0; i < len; i++) {
    var rule = rules[i], value = values[i];
...
var isArray = function (obj) { return Array.isArray ? Array.isArray(obj) : Object.prototype.toString.call(obj) === '[object Array]'; };
...
var sum = function (arr) { return arr.reduce(function (acc, val) { return acc + val; }, 0); };
...
var calculator = function (
  ...
  result.aggregate.score = util.sum(
    util.values(result.score).concat( util.values(result.bonus))
  );
